...
async def request_with_phone(update: Update, _: ContextTypes.DEFAULT_TYPE) -> None:
    """Checks if Customer's PhoneNumber is in Database before sending request to Customer Service
    if not asks for contact details permission and then sends a CONTACT message to bot"""
    logger_req.info("request_with_phone()")

    try:
        phone_number = tldb.get_customer_data(update.effective_user.id)[4]
