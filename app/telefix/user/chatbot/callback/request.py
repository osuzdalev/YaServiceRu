...
from ....daemons.prompt_validator import (
    validate_prompt,
...
class RequestCallback:
    ...
    def get_event(
        ...
        # Check if user paid for chatbot service
        else:
    ...
    async def _first_time_user(
        ...
        if conversation_size_check:
            ...
            await context.bot.delete_message(update.effective_chat.id, loading_gif.id)
            await update.message.reply_text(
                f"Conversation too long: {conversation_tokens} tokens (MAX {self._config.model.limit_conversation_tokens})"
            )
